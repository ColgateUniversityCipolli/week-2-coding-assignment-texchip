...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{(trap.area} \hlkwb{<-} \hldef{(}\hlnum{1}\hlopt{/}\hlnum{2}\hldef{)}\hlopt{*}\hlkwd{sum}\hldef{(delta.x}\hlopt{*}\hldef{(}\hlkwd{integrand}\hldef{(left.points)}\hlopt{+}\hlkwd{integrand}\hldef{(right.points))))}
\end{alltt}
\begin{verbatim}
## [1] 8.6664
\end{verbatim}
...
\hldef{riemann.sums} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{fnct}\hldef{,}                        \hlcom{# function to integrate}
  ...
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Trapezoidial"}\hldef{)\{}
    \hldef{(area} \hlkwb{<-} \hldef{(}\hlnum{1}\hlopt{/}\hlnum{2}\hldef{)}\hlopt{*}\hlkwd{sum}\hldef{(delta.x}\hlopt{*}\hldef{(}\hlkwd{integrand}\hldef{(left.points)}\hlopt{+}\hlkwd{integrand}\hldef{(right.points))))}
